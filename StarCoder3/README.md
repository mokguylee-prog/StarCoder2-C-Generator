...
왼쪽: 웹 대시보드 (`localhost:8888`) — 서버 상태·요청 통계·히스토리 실시간 확인  
오른쪽: GUI 클라이언트 — 명령 입력 / 결과 복사 / 전체 응답 3-패널 구성
...
```powershell
# 1. D드라이브에 가상환경 + 환경변수 설정
.\scripts\setup_d_drive.ps1

# 2. 모델 다운로드 (Qwen2.5-Coder-7B, ~4.4GB)
D:\StarCoder3\venv\Scripts\python.exe scripts\download_model.py
```
...
빌드 완료 후 `StarCoder3Client.exe` 생성됨 (서버와 별도 배포 가능)
...
| 모델 경로 | `D:/StarCoder3/models/gguf/` |
| 컨텍스트 길이 | 4096 토큰 |
| CPU 스레드 | 8 (조정 가능) |
| GPU 레이어 | 0 (CPU 전용) |

서버 시작 시 `D:/StarCoder3/models/gguf/` 폴더에서 가장 큰 `.gguf` 파일을 자동 선택합니다.
